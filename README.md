...
5. [Environment Setup](#-environment-setup)
   - [🪟 Windows — Complete Setup](#-windows--complete-environment-setup)
   - [🍎 macOS — Complete Setup](#-macos--complete-environment-setup)
   - [🐧 Linux — Complete Setup](#-linux--complete-environment-setup)
   - [Verify Everything Works](#-verify-everything-is-working-all-platforms)
6. [Downloading Models](#-downloading-models)
7. [Project Structure](#-project-structure)
8. [Full Benchmark Script](#-full-benchmark-script)
9. [How to Run](#-how-to-run)
10. [Output Files Explained](#-output-files-explained)
11. [Charts Generated](#-charts-generated)
12. [Configuration Reference](#-configuration-reference)
13. [Troubleshooting](#-troubleshooting)
14. [Research Findings Summary](#-research-findings-summary)
...
Choose the setup path that matches your OS. Complete all steps before pulling any models.
...
2. Click **"Download Python 3.11.x"** (the latest 3.11 release, 64-bit)
3. Run the downloaded `.exe`
4. **⚠️ CRITICAL: Check "Add Python to PATH"** at the bottom of the installer before clicking Install Now
...
If `python` is not found after install:
```cmd
# Manually add Python to PATH — open this in Command Prompt as Administrator:
...
python -m venv llm_bench_env

:: Activate it
llm_bench_env\Scripts\activate

:: You should see (llm_bench_env) at the start of your prompt
...
3. Double-click the installer — no administrator rights required
4. Ollama installs and **starts automatically as a Windows background service**
5. The Ollama icon appears in your **system tray** (bottom-right corner of taskbar)
...
> **Firewall prompt:** Windows Firewall may ask if Ollama can access the network. Click **Allow** — Ollama only listens on localhost and is not exposed to the internet.

#### GPU Support on Windows (NVIDIA)

If you have an NVIDIA GPU, Ollama uses CUDA automatically — inference will be 10–20× faster:
```cmd
:: Check your driver version (must be 525 or higher)
nvidia-smi

:: If driver is outdated, download from:
:: https://www.nvidia.com/Download/index.aspx
```

No extra Ollama config needed — it detects CUDA automatically.

#### Starting/Stopping Ollama on Windows
...
:: Or just right-click the system tray icon → Quit, then relaunch from Start Menu
```

---

### 🍎 macOS — Complete Environment Setup

#### Step 1 — Install Python 3.11

**Option A — Official installer (easiest):**
1. Go to **https://www.python.org/downloads/macos/**
2. Download **Python 3.11.x macOS 64-bit universal2 installer**
...
# Verify (on macOS use python3 / pip3)
python3 --version
pip3 --version
```
...
# Your prompt now shows (llm_bench_env)
...
**Option A — App download (easiest):**
1. Go to **https://ollama.com/download**
2. Download the macOS `.zip`
3. Open the zip and drag **Ollama.app** into your **Applications** folder
4. Launch Ollama from Applications — it appears in your **menu bar** (top-right)
5. Ollama now runs as a background service and starts automatically on login
...
# Or start as a background service that auto-starts on login
...
Apple Silicon Macs (M1 / M2 / M3 / M4) use **Metal GPU acceleration automatically** — no setup needed. Ollama detects the Neural Engine and GPU cores and uses them by default, giving you 8–15× faster inference than CPU-only. Intel Macs run CPU-only.

---

### 🐧 Linux — Complete Environment Setup
...
sudo apt install -y python3.11 python3.11-venv python3.11-dev python3-pip build-essential curl
...
**Ubuntu 20.04 — if Python 3.11 is not in default repos:**
```bash
...
```bash
sudo dnf install -y python3.11 python3-pip gcc gcc-c++ make curl
```

**Arch Linux / Manjaro:**
```bash
sudo pacman -Syu python python-pip base-devel curl
```

**openSUSE:**
```bash
sudo zypper install -y python311 python311-pip curl
```
...
python3.11 -m venv llm_bench_env
...
#### Step 3 — Install Ollama on Linux

**One-line installer (Ubuntu, Debian, Fedora, Arch, openSUSE):**
```bash
...
- Creates a `systemd` service so Ollama starts on every boot
- Sets up a dedicated `ollama` system user

**Control the Ollama service:**
```bash
# Start immediately
...
**Manual install (if the script fails or you prefer no systemd):**
```bash
...
# Run manually (keep this terminal open, or use nohup / screen / tmux)
ollama serve
```

**Run Ollama without sudo (recommended):**
```bash
# Add your user to the ollama group (created by the installer)
...
# After reboot, verify CUDA is available
nvidia-smi
...
```bash
# Follow official Ollama AMD guide:
# https://ollama.com/blog/amd-preview
# Requires ROCm 5.7+ and an RDNA2+ GPU (RX 6000 series or newer)
```

**Check which GPU Ollama is using:**
```bash
# Start ollama serve and watch the startup logs
ollama serve 2>&1 | head -20
# Look for lines like "using CUDA device" or "using Metal"
```

---

### ✅ Verify Everything Is Working (All Platforms)

Run these commands in order after completing your OS-specific setup:

```bash
# 1. Check Ollama API is live
curl http://localhost:11434/api/tags
# Expected: {"models":[]}
...
# 4. Check your virtual environment is active
# Your terminal prompt should start with (llm_bench_env)
# If not, activate it:
source llm_bench_env/bin/activate   # macOS / Linux
llm_bench_env\Scripts\activate      # Windows

# 5. Quick test — run a model interactively
...
# Create environment
conda create -n llm_bench python=3.11 -y
...
### Option C — requirements.txt (All Platforms)

Save this file as `requirements.txt` in your project folder:
...
### Text Models (Required — pick at least one)

```bash
# LLaMA3 8B — Best quality, recommended (4.7 GB download)
ollama pull llama3

# Mistral 7B — Fast and lightweight (4.1 GB download)
ollama pull mistral

# LLaMA2 7B — Fallback if above unavailable (3.8 GB download)
ollama pull llama2

# Gemma 7B — Google's model (5.0 GB download)
ollama pull gemma
```

> **Note:** The script auto-detects which text model you have. Priority order: `llama3` → `mistral` → `llama2` → `gemma`. You only need **one**.

### Image Generation Model (Optional — macOS only)

```bash
# FLUX.2 Klein 4B — Black Forest Labs diffusion model (~4 GB download)
# IMPORTANT: Only works on macOS via Ollama's experimental image API
ollama pull x/flux2-klein

# If x/flux2-klein is unavailable, try:
ollama pull x/zimage-turbo
```

> **If you're on Windows or Linux:** The script will automatically skip the image generation case and run only the two text cases. You will still get all charts and the full report for the cases that ran.
...
# x/flux2-klein:latest  abc123def456    3.9 GB  30 minutes ago
```
...
|-------|------|-----|----------|
| `llama3` | 4.7 GB | Short + Long text | All |
| `mistral` | 4.1 GB | Short + Long text | All |
| `llama2` | 3.8 GB | Short + Long text | All |
| `gemma` | 5.0 GB | Short + Long text | All |
| `x/flux2-klein` | ~4 GB | Image generation | macOS only |
...
your-project/
│
├── llm_benchmark_full.py        ← The main benchmark script (everything in one file)
├── requirements.txt             ← Python dependencies
├── README.md                    ← This file
│
└── benchmark_output/            ← Created automatically when you run the script
    ├── all_runs.csv
    ├── summary.csv
...
=======================================================
  ...
  Runs/case   : 3
=======================================================
✅ Ollama running. Models found: ['llama3:latest', 'x/flux2-klein:latest']
✅ Using text model: llama3:latest
✅ Image model found: x/flux2-klein:latest
...
=======================================================
  CASE: Image Gen  |  Model: x/flux2-klein  |  3 runs
  Resolution: 512×512  |  Steps: 20
=======================================================
  Run 1/3 ... (this takes ~60–90s on CPU) ... ✅  67.8s  |  512×512px  | ...
  ...
